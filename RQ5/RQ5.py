...
for workpath in all_workpath:
    ...
    csv_write.writerow(["pv",'MS k','MS spearman','MS pearson', 'SMS', 'SMS', 'SMS','RMS','RMS','RMS','CMS','CMS','CMS','COS','COS','COS','SC','SC','SC','BC','BC','BC'])
    resultdic[pv]=1
    result=[pv]
    ...
    for ratio in ratiolist:
        ...
        for _ in range(100):
            testlist=Sample(testmap, ratio)
            if len(set(tindexs)&set(testlist))>0:
                detected.append(1)
            else:
                detected.append(0)
            MS.append(computeMS(labelarray,testlist,allmutant))
            SMS.append(computeMS(labelarray,testlist,SMSmutant))
            RMS.append(computeMS(labelarray,testlist,RMSmutant))
            CMS.append(computeMS(labelarray,testlist,CMSmutant))
            COS.append(computeMS(labelarray,testlist,COSmutant))
            SC.append(getSC(coverarray,testlist))
            BCworkpath=workpath.replace('statement',"branch")
            if os.path.exists(BCworkpath):
                BC.append(getBC(cwdpath,testlist,BCworkpath))
                print("BC:"+str(BC[-1]))
            print([MS[-1],SMS[-1],RMS[-1],CMS[-1],COS[-1],SC[-1]])
        ans=[]
        #ans的结果分别是：MS的kendall，spearman,pearson. SMS,RMS,CMS,COS,SC,BC
        ans.append(computeKendall(detected,MS))
        ans.append(computeSpearmanr(detected,MS))
        ans.append(computePearsonr(detected,MS))
        ans.append(computeKendall(detected,SMS))
        ans.append(computeSpearmanr(detected,SMS))
        ans.append(computePearsonr(detected,SMS))
        ans.append(computeKendall(detected,RMS))
        ans.append(computeSpearmanr(detected,RMS))
        ans.append(computePearsonr(detected,RMS))
        ans.append(computeKendall(detected,CMS))
        ans.append(computeSpearmanr(detected,CMS))
        ans.append(computePearsonr(detected,CMS))
        ans.append(computeKendall(detected,COS))
        ans.append(computeSpearmanr(detected,COS))
        ans.append(computePearsonr(detected,COS))
        ans.append(computeKendall(detected,SC))
        ans.append(computeSpearmanr(detected,SC))
        ans.append(computePearsonr(detected,SC))
        if len(BC)==len(detected):
            ans.append(computeKendall(detected,BC))
            ans.append(computeSpearmanr(detected,BC))
            ans.append(computePearsonr(detected,BC))
        else:
            ans.append(-1)
            ans.append(-1)
            ans.append(-1)
